<?php
session_start();

if (!isset($_SESSION['client_id'])) {
    header('Location: index.php');
    exit();
}

$client_id = $_SESSION['client_id'];

try {
    $conn = new PDO("mysql:host=localhost;dbname=lalampemagique;charset=utf8", "root", "");
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
} catch (PDOException $e) {
    echo "Connection failed: " . $e->getMessage();
    exit();
}

$query = "UPDATE commande SET STATUT = 'Completed' WHERE ID_CLIENT = ? AND STATUT = 'En cours'";
$stmt = $conn->prepare($query);
$stmt->execute([$client_id]);

$query = "SELECT cp.ID_PRODUIT, cp.QUANTITE FROM commande_produit cp
          JOIN commande c ON cp.ID_COMMANDE = c.ID_COMMANDE
          WHERE c.ID_CLIENT = ? AND c.STATUT = 'Completed'";
$stmt = $conn->prepare($query);
$stmt->execute([$client_id]);
$items = $stmt->fetchAll(PDO::FETCH_ASSOC);

foreach ($items as $item) {
    $query = "UPDATE produit SET STOCK = STOCK - ? WHERE ID_PRODUIT = ?";
    $stmt = $conn->prepare($query);
    $stmt->execute([$item['QUANTITE'], $item['ID_PRODUIT']]);
}

$query = "DELETE cp FROM commande_produit cp
          JOIN commande c ON cp.ID_COMMANDE = c.ID_COMMANDE
          WHERE c.ID_CLIENT = ? AND c.STATUT = 'Completed'";
$stmt = $conn->prepare($query);
$stmt->execute([$client_id]);

$conn = null;

header('Location: confirmation.php');
exit();
?>
